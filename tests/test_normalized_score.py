    ...
    def testNormalizations(self):
        for sc in np.arange(0, 2, 0.05):
            for bsc in np.arange(0.5, 1, 0.05):
                ...
                assert score._normalize(sc, bsc, transformation_true) == pytest.approx((bsc - sc) / abs(bsc), 1e-8)
                assert score._normalize(sc, bsc, transformation_false) == pytest.approx((sc - bsc) / abs(bsc), 1e-8)
