...
PREDICTED_OK_REAL_EXAMPLE = ['value_a', 'value_b', 'value_a', 'value_c',
                'value_a', 'value_a', 'value_a', 'value_b',
...
class TestMXE(unittest.TestCase):
    ...
    def testBest(self):
        ...
        self.assertAlmostEqual(METRICS_DICT['crossEntropy'](GROUND_TRUTH, PREDICTED_BEST_PROB, CLASS_ORDER), 0.0)
        self.assertAlmostEqual(METRICS_DICT['crossEntropyNonBinarized'](GROUND_TRUTH, PREDICTED_BEST), 0.0)

        self.assertAlmostEqual(mxe(GROUND_TRUTH_4, PREDICTED_BEST_PROB_4, CLASS_ORDER_4), 0.0)
        self.assertAlmostEqual(METRICS_DICT['crossEntropy'](GROUND_TRUTH_4, PREDICTED_BEST_PROB_4, CLASS_ORDER_4), 0.0)
    ...
    def testOk(self):
        ...
        self.assertAlmostEqual(mxe(GROUND_TRUTH, PREDICTED_OK_PROB, CLASS_ORDER), expected_OK_mxe_prob)
        self.assertAlmostEqual(METRICS_DICT['crossEntropy'](GROUND_TRUTH, PREDICTED_OK_PROB, CLASS_ORDER), expected_OK_mxe_prob)
        ...
        self.assertAlmostEqual(mxe(GROUND_TRUTH_4, PREDICTED_OK_PROB_4, CLASS_ORDER_4), expected_OK4_mxe_prob)
        self.assertAlmostEqual(METRICS_DICT['crossEntropy'](GROUND_TRUTH_4, PREDICTED_OK_PROB_4, CLASS_ORDER_4), expected_OK4_mxe_prob)
    ...
    def testOkRealExample(self):
        ...
        expected_OK_mxe_prob = (1/3)*((1/5)*(4*ct_mxe + 1*it_mxe) + (1/4)*(3*ct_mxe + 1*it_mxe) + (1/3)*(2*ct_mxe + 1*it_mxe))
        expected_OK_mxe_bin = (-math.log(2**-100,2)/3)*(1/5 + 1/4 + 1/3)
        self.assertAlmostEqual(mxe(GROUND_TRUTH_REAL_EXAMPLE, PREDICTED_OK_PROB, CLASS_ORDER_REAL_EXAMPLE), expected_OK_mxe_prob)
        self.assertAlmostEqual(METRICS_DICT['crossEntropy'](GROUND_TRUTH_REAL_EXAMPLE, PREDICTED_OK_PROB, CLASS_ORDER_REAL_EXAMPLE), expected_OK_mxe_prob)
        self.assertAlmostEqual(METRICS_DICT['crossEntropyNonBinarized'](GROUND_TRUTH_REAL_EXAMPLE, PREDICTED_OK_REAL_EXAMPLE), expected_OK_mxe_bin)

    def testBad(self):
        eps = 2**-100 # log2(0) is undefined, so mxe uses log2(eps) instead of log2(0) Log2 is the base, which is what prompts the epsilon
        self.assertAlmostEqual(mxe(GROUND_TRUTH, PREDICTED_WORST_PROB, CLASS_ORDER), -math.log2(eps))
        self.assertAlmostEqual(METRICS_DICT['crossEntropy'](GROUND_TRUTH, PREDICTED_WORST_PROB, CLASS_ORDER), -math.log2(eps))
        self.assertAlmostEqual(METRICS_DICT['crossEntropyNonBinarized'](GROUND_TRUTH, PREDICTED_BAD), -math.log2(eps))

        self.assertAlmostEqual(mxe(GROUND_TRUTH_4, PREDICTED_WORST_PROB_4, CLASS_ORDER_4), -math.log2(eps))
        self.assertAlmostEqual(METRICS_DICT['crossEntropy'](GROUND_TRUTH_4, PREDICTED_WORST_PROB_4, CLASS_ORDER_4), -math.log2(eps))
    ...
    def testBaseballEx1(self):
        ...
        self.assertAlmostEqual(mxe(GROUND_TRUTH_BBALL,PREDICTED_BBALL_PROBS, CLASS_ORDER_BBALL), expected_mxe)
        self.assertAlmostEqual(METRICS_DICT['crossEntropyNonBinarized'](GROUND_TRUTH_BBALL, PREDICTED_BBALL), expected_mxe)
        self.assertAlmostEquals(mxe(GROUND_TRUTH_BBALL,PREDICTED_BBALL_PROBS, CLASS_ORDER_BBALL),METRICS_DICT['crossEntropyNonBinarized'](GROUND_TRUTH_BBALL, PREDICTED_BBALL), expected_mxe)
