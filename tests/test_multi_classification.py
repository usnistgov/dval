...
A = [0.9, 0.05, 0.025, 0.025]
B = [0.05, 0.9, 0.025, 0.025]
C = [0.05, 0.025, 0.9, 0.025]
D = [0.05, 0.025, 0.025, 0.9]
...
CLASS_ORDER_4 = ["a", "b", "c", "d"]

GROUND_TRUTH_REAL_EXAMPLE = [
    "value_a",
    "value_b",
    "value_a",
    "value_b",
...
CLASS_ORDER_REAL_EXAMPLE = ["value_a", "value_b", "value_c"]
...
class TestMXE(unittest.TestCase):
    ...
    def testBest(self):
        ...
        self.assertAlmostEqual(mxe(GROUND_TRUTH, PREDICTED_BEST_PROB, CLASS_ORDER), 0.0)
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropy"](
                GROUND_TRUTH, PREDICTED_BEST_PROB, CLASS_ORDER
            ),
            0.0,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](GROUND_TRUTH, PREDICTED_BEST), 0.0
        )

        self.assertAlmostEqual(
            mxe(GROUND_TRUTH_4, PREDICTED_BEST_PROB_4, CLASS_ORDER_4), 0.0
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropy"](
                GROUND_TRUTH_4, PREDICTED_BEST_PROB_4, CLASS_ORDER_4
            ),
            0.0,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](GROUND_TRUTH_4, PREDICTED_BEST_4),
            0.0,
        )
    ...
    def testOk(self):
        ...
        expected_OK_mxe_prob = (1 / 3) * (
            (1 / 5) * (4 * ct_mxe + 1 * it_mxe)
            + (1 / 4) * (3 * ct_mxe + 1 * it_mxe)
            + (1 / 3) * (2 * ct_mxe + 1 * it_mxe)
        )
        expected_OK_mxe_bin = (-math.log(2 ** -100, 2) / 3) * (1 / 5 + 1 / 4 + 1 / 3)
        self.assertAlmostEqual(
            mxe(GROUND_TRUTH, PREDICTED_OK_PROB, CLASS_ORDER), expected_OK_mxe_prob
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropy"](GROUND_TRUTH, PREDICTED_OK_PROB, CLASS_ORDER),
            expected_OK_mxe_prob,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](GROUND_TRUTH, PREDICTED_OK),
            expected_OK_mxe_bin,
        )

        ct4_mxe = math.log2(1 / 0.9)
        it4ab_mxe = math.log2(1 / 0.05)
        expected_OK4_mxe_prob = (1 / 4) * (
            (1 / 5) * (4 * ct4_mxe + 1 * it4ab_mxe)
            + (1 / 3) * (3 * ct4_mxe)
            + (1 / 3) * (3 * ct4_mxe)
            + (1 / 2) * (2 * ct4_mxe)
        )
        ...
        self.assertAlmostEqual(
            mxe(GROUND_TRUTH_4, PREDICTED_OK_PROB_4, CLASS_ORDER_4),
            expected_OK4_mxe_prob,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropy"](
                GROUND_TRUTH_4, PREDICTED_OK_PROB_4, CLASS_ORDER_4
            ),
            expected_OK4_mxe_prob,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](GROUND_TRUTH_4, PREDICTED_OK_4),
            expected_OK4_mxe_bin,
        )
    ...
    def testOkRealExample(self):
        ...
        expected_OK_mxe_bin = (-math.log(2 ** -100, 2) / 3) * (1 / 5 + 1 / 4 + 1 / 3)
        self.assertAlmostEqual(
            mxe(GROUND_TRUTH_REAL_EXAMPLE, PREDICTED_OK_PROB, CLASS_ORDER_REAL_EXAMPLE),
            expected_OK_mxe_prob,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropy"](
                GROUND_TRUTH_REAL_EXAMPLE, PREDICTED_OK_PROB, CLASS_ORDER_REAL_EXAMPLE
            ),
            expected_OK_mxe_prob,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](
                GROUND_TRUTH_REAL_EXAMPLE, PREDICTED_OK_REAL_EXAMPLE
            ),
            expected_OK_mxe_bin,
        )
    ...
    def testBad(self):
        eps = (
            2 ** -100
        )  # log2(0) is undefined, so mxe uses log2(eps) instead of log2(0) Log2 is the base, which is what prompts the epsilon
        self.assertAlmostEqual(
            mxe(GROUND_TRUTH, PREDICTED_WORST_PROB, CLASS_ORDER), -math.log2(eps)
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropy"](
                GROUND_TRUTH, PREDICTED_WORST_PROB, CLASS_ORDER
            ),
            -math.log2(eps),
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](GROUND_TRUTH, PREDICTED_BAD),
            -math.log2(eps),
        )

        self.assertAlmostEqual(
            mxe(GROUND_TRUTH_4, PREDICTED_WORST_PROB_4, CLASS_ORDER_4), -math.log2(eps)
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropy"](
                GROUND_TRUTH_4, PREDICTED_WORST_PROB_4, CLASS_ORDER_4
            ),
            -math.log2(eps),
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](GROUND_TRUTH_4, PREDICTED_WORST_4),
            -math.log2(eps),
        )
    ...
    def testBaseballEx1(self):
        ...
        self.assertAlmostEqual(
            mxe(GROUND_TRUTH_BBALL, PREDICTED_BBALL_PROBS, CLASS_ORDER_BBALL),
            expected_mxe,
        )
        self.assertAlmostEqual(
            METRICS_DICT["crossEntropyNonBinarized"](
                GROUND_TRUTH_BBALL, PREDICTED_BBALL
            ),
            expected_mxe,
        )
...
class TestF1Macro(unittest.TestCase):
    ...
    def testOk(self):
        ...
        self.assertEqual(
            0.73888888888888893, METRICS_DICT["f1Macro"](GROUND_TRUTH, PREDICTED_OK)
        )
...
class TestROCAUCMicro(unittest.TestCase):
    ...
    def testOk(self):
        ...
        self.assertEqual(
            0.8125, METRICS_DICT["rocAucMicro"](GROUND_TRUTH, PREDICTED_OK)
        )
...
class TestROCAUCMacro(unittest.TestCase):
    ...
    def testOk(self):
        ...
        self.assertEqual(
            0.80628306878306877, METRICS_DICT["rocAucMacro"](GROUND_TRUTH, PREDICTED_OK)
        )

    def testBad(self):
        self.assertEqual(
            0.25033068783068785, roc_auc_macro(GROUND_TRUTH, PREDICTED_BAD)
        )
        self.assertEqual(
            0.25033068783068785,
            METRICS_DICT["rocAucMacro"](GROUND_TRUTH, PREDICTED_BAD),
        )
...
class TestJaccSim(unittest.TestCase):
    ...
    def testBest(self):
        ...
        self.assertEqual(
            1.0, METRICS_DICT["jaccardSimilarityScore"](GROUND_TRUTH, PREDICTED_BEST)
        )

    def testOk(self):
        self.assertEqual(0.75, jacc_sim(GROUND_TRUTH, PREDICTED_OK))
        self.assertEqual(
            0.75, METRICS_DICT["jaccardSimilarityScore"](GROUND_TRUTH, PREDICTED_OK)
        )

    def testBad(self):
        self.assertEqual(0, jacc_sim(GROUND_TRUTH, PREDICTED_BAD))
        self.assertEqual(
            0, METRICS_DICT["jaccardSimilarityScore"](GROUND_TRUTH, PREDICTED_BAD)
        )
